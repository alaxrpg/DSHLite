...
public final class AppState {
    ...
    public private(set) var updateState: DSHUpdateState = .idle {
        didSet {
            if oldValue != updateState { notifyObservers() }
        }
    }
    public var isLogsSheetPresented = false {
        didSet { if isLogsSheetPresented { LogsPanel.show(self) } }
    }
    public var isSettingsSheetPresented = false {
        didSet { if isSettingsSheetPresented { SettingsPanel.show(self) } }
    }
    ...
    /// 构造 supervisor：所有参数从 Settings 读取（计划要求：packageSpec 必须来自配置，不硬编码）。
    @discardableResult
    ...
    public func restartBackend() {
        enqueueLifecycle { [weak self] in
            ...
            if let supervisor = self.supervisor {
                // 必须等待旧进程组已 reap，再替换 supervisor，避免旧进程/回调污染新实例。
    ...
    /// 窗口重新显示时无条件排入启动操作；若此前的 stop 尚未完成，队列会自然等待。
    ...
    @discardableResult
    private func enqueueLifecycle(_ operation: @escaping @MainActor () async -> Void) -> Task<Void, Never> {
